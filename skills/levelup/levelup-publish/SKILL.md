...
#### Phase 8: log.md Update (OKF §9)

For each accepted CDR whose module was written in Phase 5, append an entry to the relevant `context_modules/<type>/log.md` file (OKF §9 date-grouped format, newest first):

```markdown
## {today}
* **Creation**: Added [{Title}](relative-path.md) — {one-line description}. CDR: {cdr_ref}.
```

If `context_modules/<type>/log.md` does not exist, create it with a `# {Type} Update Log` heading first.

**CDR.md is no longer manually updated by publish.** It is a derived artifact, auto-generated by `/team-repair` from the per-directory `index.md` files. After merging the PR, run `/team-repair` to regenerate `CDR.md`.
...
- Published files include YAML frontmatter with OKF v0.2 families: `generated`, `verified` (list), `status`, `stale_after`
- `log.md` files track per-directory audit trail (OKF §9)
- `CDR.md` is derived by `/team-repair` from `index.md` files — not manually maintained

#### Context Modules Before log.md

- **CRITICAL**: Create ALL context module and skill files before appending to `log.md`
- Do not write `log.md` entries first and skip the actual modules
...
- Migrate any remaining v0.1 frontmatter to v0.2 (always-on)
- Rebuild per-directory `index.md` and `log.md` files
- Re-derive `CDR.md` flat table from `index.md` files
...
- Context module files created in `{TEAM_AI_DIRECTIVES}/context_modules/` with OKF v0.2 frontmatter (`generated`, `verified` list, `status`, `stale_after`)
- Per-directory `index.md` entries appended (OKF §8 list format)
- Per-directory `log.md` entries appended (OKF §9 date-grouped format)
...
- **After merge**: run `/team-repair` to regenerate derived `CDR.md` from `index.md` files
