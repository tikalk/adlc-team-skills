...
- Regenerate `pdr.md` index (drafts)

**Step 4.2b: Generate Memory PDR Index (MANDATORY)**

After moving Accepted PDRs to `.adlc/memory/pdr/`, generate a memory index file at `{REPO_ROOT}/.adlc/memory/pdr.md` from all `PDR-*.md` files in `.adlc/memory/pdr/`.

Scan each `PDR-*.md` file in `.adlc/memory/pdr/` and extract: ID, Feature-Area, Category, Status, Date, Owner, Title (from frontmatter or first heading).

Write `{REPO_ROOT}/.adlc/memory/pdr.md`:
...
|----|--------------|----------|--------|------|-------|-------|
| PDR-001 | system | Target Market | Accepted | 2026-03-09 | User/AI | ... |
```
...
| 8 | Memory PDR index generated | `.adlc/memory/pdr.md` exists with correct table |
