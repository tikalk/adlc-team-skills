...
type: product  # product | decision | constraint
evidence: confirmed  # confirmed | inferred | unknown
source: ""  # where the rationale came from (session YYYY-MM-DD, interview, issue, commit)
revisit-when: ""  # concrete trigger that should prompt re-checking this PDR
---
...
{A concrete trigger that should prompt re-checking this PDR. Age alone is
not a condition. Leave as "N/A" if no specific trigger applies.}
