...
def check_sweep_layer_routing(output: str) -> dict:
    """ADR-361: Sweep findings route to the matching track's clarify by layer tag."""
    m = re.search(r"layer:\s*(product|architecture|change|cross)\b", output, re.IGNORECASE)
    if not m:
        return { "pass": False, "score": 0.0, "reason": f"Failed ADR-361: No [layer:] tag on the finding — cannot route to the matching track clarify, but got: {output}" }
    ...
    if not has_track_clarify:
        return { "pass": False, "score": 0.0, "reason": f"Failed ADR-361: No named track clarify (product-clarify/architect-clarify/change-clarify) in the routing, but got: {output}" }
    if layer == "cross":
        return { "pass": True, "score": 1.0, "reason": "Passed ADR-361: Cross-layer finding routed to a track clarify (cross findings route to the owning track)." }
