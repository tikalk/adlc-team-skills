...
type: decision  # decision | workaround | constraint | abandoned
evidence: confirmed  # confirmed | inferred | unknown — how well the origin is established
source: ""  # where the rationale came from (session, interview, issue, commit, post-mortem)
revisit-when: ""  # concrete trigger that should prompt re-checking this ADR
---
...
{A concrete trigger that should prompt re-checking this ADR — e.g., "retry
mechanism changes", "gateway SDK major version bump". Age alone is not a
condition. Leave as "N/A" if no specific trigger applies.}
