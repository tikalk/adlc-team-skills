---
description: Generate full Product Requirements Document (PRD) from PDRs using multi-agent
  DAG orchestration with mandatory checkpoint after Requirements
version: 1.5.6
compliance: strict
scripts:
  sh: .specify/extensions/product/scripts/bash/setup-product.sh "implement {ARGS}"
  ps: .specify/extensions/product/scripts/powershell/setup-product.ps1 "implement
    {ARGS}"
---


<!-- Extension: product -->
<!-- Config: .specify/extensions/product/ -->
---
...
- [ ] **MUST** read and use templates from `.specify/templates/sections/{section}.md` - **DO NOT generate from scratch**
...
- [ ] **MUST** validate output with `.specify/scripts/validate-prd.sh --strict` after each section

### STRICT ENFORCEMENT RULES:

1. **Template Usage is MANDATORY**
   - Read the section template FIRST: `cat .specify/templates/sections/{section}.md`
...
3. **LOAD SECTION TEMPLATE (STRICT - MUST DO)**: 
   - **MUST** read: `cat .specify/templates/sections/{section}.md`
...
- If `.specify/memory/pdr.md` already exists, merge (append new PDRs, update existing)
...
| 8 | **Memory PDRs written** | `.specify/memory/pdr.md` exists | `ls .specify/memory/pdr.md` | ☐ |
...
- **Check 6 is the most commonly skipped** — verify `.specify/memory/pdr.md` actually exists on disk.
