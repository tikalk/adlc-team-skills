...
$Features = @()
foreach ($searchDir in @((Join-Path $ProjectRoot "specs"))) {
    if (Test-Path $searchDir) {
        Get-ChildItem -Path $searchDir -Directory -ErrorAction SilentlyContinue | ForEach-Object { $Features += $_.FullName }
    }
}
# Also search immediate subdirectories for monorepo layouts (e.g., {subproject}/specs/)
Get-ChildItem -Path $ProjectRoot -Directory -ErrorAction SilentlyContinue | ForEach-Object {
    $subSpecs = Join-Path $_.FullName "specs"
    if (Test-Path $subSpecs) {
        Get-ChildItem -Path $subSpecs -Directory -ErrorAction SilentlyContinue | ForEach-Object { $Features += $_.FullName }
    }
}
