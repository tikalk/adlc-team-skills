#!/usr/bin/env bash
# setup-levelup-specify.sh — Setup for levelup-specify
set -euo pipefail
...
detect_features() {
  ...
  if [[ -d "${PROJECT_ROOT}/specs" ]]; then
    while IFS= read -r d; do
      [[ -n "$d" ]] && features+=("$d")
    done < <(find "${PROJECT_ROOT}/specs" -maxdepth 1 -mindepth 1 -type d 2>/dev/null | sort)
  fi
  printf '%s\n' "${features[@]}"
}
...
python3 - "$PROJECT_ROOT" "$CDR_DRAFTS_DIR" "$SKILLS_DRAFTS_DIR" "$LEVELUP_STATE_FILE" "$TEAM_DIRECTIVES" "$BRANCH" "$NEXT_CDR" "$EXISTING_CDRS" "$TD_CONFIGURED" "$FEATURES_JSON" << 'PY'
import json, sys
print(json.dumps({
