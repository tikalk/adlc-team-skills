...
resolve_levelup_paths() {
  ...
  if declare -f resolve_paths >/dev/null; then
    eval "$(resolve_paths 2>/dev/null | grep -E '^(PROJECT_ROOT|TEAM_AI_DIRECTIVE|BRANCH)=')" || true
  else
    TEAM_AI_DIRECTIVE="${TEAM_AI_DIRECTIVE:-}"
    if [[ -z "$TEAM_AI_DIRECTIVE" && -f "${PROJECT_ROOT}/.adlc/init-options.json" ]]; then
...
print(json.dumps({
  ...
  "TEAM_AI_DIRECTIVE": os.environ.get("TEAM_AI_DIRECTIVE", ""),
...

  {
    ...
    if [[ -d "$dir" ]]; then
      for f in $(ls -1 "$dir"/CDR-*.md 2>/dev/null | sort); do
        ...
        id=$(basename "$f" .md)
        target=$(grep -m1 '^### Target Module:' "$f" 2>/dev/null | sed 's/^### Target Module:[[:space:]]*//' | sed 's/`//g' | head -c 40 || true)
        type=$(grep -m1 '^### Context Type:' "$f" 2>/dev/null | sed 's/^### Context Type:[[:space:]]*//' | head -c 20 || true)
        status=$(grep -m1 '^### Status:' "$f" 2>/dev/null | sed 's/^### Status:[[:space:]]*//' | sed 's/\*\*//g' | head -c 12 || true)
        created=$(grep -m1 '^### Date:' "$f" 2>/dev/null | sed 's/^### Date:[[:space:]]*//' | head -c 10 || true)
        verified=$(grep -m1 '^verified:' "$f" 2>/dev/null | sed 's/verified:[[:space:]]*//' | head -c 10 || true)
        age=$(grep -m1 '^age_days:' "$f" 2>/dev/null | sed 's/age_days:[[:space:]]*//' | head -c 5 || true)
        descriptor=$(grep -m1 '^### Descriptor:' "$f" 2>/dev/null | sed 's/^### Descriptor:[[:space:]]*//' | head -c 60 || true)
...
scan_rule_conflicts() {
  ...
  if [[ -z "$rules_dir" ]]; then
    resolve_levelup_paths >/dev/null
    rules_dir="${TEAM_AI_DIRECTIVE}/context_modules/rules"
  fi
