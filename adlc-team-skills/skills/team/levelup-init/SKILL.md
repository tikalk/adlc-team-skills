...
- Scan the codebase for reusable rules, personas, examples, and skill-worthy capabilities
...
```bash
skills/levelup/levelup-init/scripts/bash/setup-levelup-init.sh
```

Parse the JSON output for `REPO_ROOT`, `CDR_DRAFTS_DIR`, `TEAM_DIRECTIVES`, `NEXT_CDR`, etc.

If `TEAM_DIRECTIVES` is not configured:
...
### Context Type
Rule | Persona | Example | Skill | Constitution Creation | Constitution Amendment

### Descriptor
One-line "when to use" summary for CDR index search.
...
1. Run the helper to regenerate the index:
