...
- **Trailing whitespace stripped from architect AD templates** (`skills/architect/*/templates/`): 35 template files across all 5 architect skills (architect-analyze, -clarify, -implement, -init, -specify) had 190 trailing-whitespace lines that failed `git diff --check` in downstream repos. Whitespace-only blank lines inside mermaid blocks were stripped to empty; 7 markdown hard-breaks per `AD-template.md` (2-space) were converted to backslash (renders identically on GitHub, passes both `git diff --check` and markdownlint MD009).
