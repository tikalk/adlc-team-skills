...
2. Verify:
   ...
   - `{TEAM_DIRECTIVES}/context_modules/personas/`
   - `{TEAM_DIRECTIVES}/context_modules/rules/`
   - `{TEAM_DIRECTIVES}/context_modules/examples/`

Output: `[OK]` or `[FAIL]` with reason

#### Check 4: Skills Registry

- `{TEAM_DIRECTIVES}/.skills.json` exists and is valid JSON

Output: `[OK]` or `[FAIL]` with reason

#### Check 5: CDR Tracking

- `{TEAM_DIRECTIVES}/CDR.md` exists

Output: `[OK]` or `[FAIL]` with reason

#### Check 6: Constitution Alignment

1. Read team constitution from `{TEAM_DIRECTIVES}/context_modules/constitution.md`
...
- **`{TEAM_DIRECTIVES}/.skills.json` is missing or not valid JSON**: skill discovery is broken; agents cannot find team skills even if the files exist.
...
```bash
cp "templates/agents-template.md" "{TEAM_DIRECTIVES}/AGENTS.md"
```
...
2. Determine context type from path:
   ...
   - `personas/` → `Persona`
   - `examples/` → `Example`
3. Compute the file's relative path from `TEAM_DIRECTIVES` and look it up in `CDR_LOOKUP`:
...
- **Proceeding past Phase 2 when `TEAM_DIRECTIVES` is empty** — operating without a configured repository writes to undefined paths and corrupts the wrong workspace.
...
- [ ] AGENTS.md exists at `{TEAM_DIRECTIVES}/AGENTS.md` and contains all six required sections.
