...
root — the directory where `.adlc/` lives) and `{TEAM_DIRECTIVES}/.skills.json`
...
```
Team Skills from {TEAM_DIRECTIVES}
...
1. Locate `{TEAM_DIRECTIVES}/skills/{name}/SKILL.md`
...
- Both `{REPO_ROOT}/.skills.json` and `{TEAM_DIRECTIVES}/.skills.json` were merged; project-root entries won any conflicts.
