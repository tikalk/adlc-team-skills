...
for arg in "$@"; do
    case "$arg" in
        ...
        --help|-h)
            ...
            echo "  $0 .adlc/product/sections/ecosystem/overview.md  # Validate section"
            exit 0
            ;;
    esac
done
...
# ============================================
echo ""
echo -e "${BLUE}[6.6/9] Checking self-contained PRD (v1.5.6 - no external .adlc/ links)...${NC}"

# Check for reader-facing links to .adlc/ paths
EXTERNAL_LINKS=$(grep -nE '\]\(\.adlc/' "$PRD_FILE" 2>/dev/null || true)
...
if [[ -n "$EXTERNAL_LINKS_FILTERED" ]]; then
    warn "PRD contains reader-facing links to .adlc/ files (should be self-contained)"
    ...
    pass "PRD is self-contained (no reader-facing .adlc/ links)"
fi
