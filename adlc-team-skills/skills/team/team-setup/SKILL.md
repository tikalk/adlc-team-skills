...
# Update or add team_ai_directives field
echo "$CONFIG" | python3 -c "
import json, sys
config = json.load(sys.stdin)
config['team_ai_directives'] = '$ABSOLUTE_PATH'
...
3. Check `ADLC_TEAM_AI_DIRECTIVES` env var as fallback
...
2. Verify the KB is accessible by running a quick health check:
   - `{TEAM_DIRECTIVES}/context_modules/constitution.md` exists
...
- **Skipping the `team_ai_directives` config write** — without this field in `init-options.json`, agents cannot discover the KB.
...
- [ ] `{TEAM_DIRECTIVES}/context_modules/constitution.md` exists.
- [ ] `{TEAM_DIRECTIVES}/context_modules/rules/` exists.
- [ ] `{TEAM_DIRECTIVES}/context_modules/personas/` exists.
- [ ] `{TEAM_DIRECTIVES}/context_modules/examples/` exists.
